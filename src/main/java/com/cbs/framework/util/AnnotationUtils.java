package com.cbs.framework.util;
...
public abstract class AnnotationUtils {
    ...
    public static Set<Class<?>> findAnnotatedClasses(String packageName, Class<? extends Annotation> annotationType) {
        Reflections reflections = new Reflections(packageName, new SubTypesScanner(false));
        Set<Class<?>> classes = reflections.getSubTypesOf(Object.class);
        return classes.stream()
                .filter(aClass -> isAnnotationPresent(aClass, annotationType))
        ...
        for (Method method : methods) {
            if (method.getParameterCount() == 0 && method.getReturnType() != void.class) {
                try {
                    ...
                    throw new RuntimeException("Error while inspecting annotation values", e);
                }
            }
        }
    }
